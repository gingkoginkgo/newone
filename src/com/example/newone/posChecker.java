...
public class posChecker extends Service {
	...
	private double _lng;
	private double _lat;
	private final int delaySec = 20;
	private final double EARTH_RADIUS = 6378137.0;
	private ArrayList<ToDo> _todoList = ToDoManager.getInstance().getUserToDo();
	...
	private int checkPOIbyTarget(String s) {
		...
		ArrayList<POI> _resulttmp = _ps.getNearByPOIs(_lat, _lng, s);
		...
		for (POI p : _result) {
			if (nearPOI == null) {
				nearPOI = p;
				nearDis = (p.getLat() - _lat) * (p.getLat() - _lat)
						+ (p.getLng() - _lng) * (p.getLng() - _lng);
			} else {
				curDis = (p.getLat() - _lat) * (p.getLat() - _lat)
						+ (p.getLng() - _lng) * (p.getLng() - _lng);
				if (curDis < nearDis){
					nearPOI = p;
					nearDis = curDis;
		...
		for (ToDo d : tmpl) {
			if (d.getTarget_Place().equals(s)) {
				...
				d.setIsByLoc(nearPOI); // 塞入最近的
			}
		}
		return _result.size();
	}

	private void gNotification(String Title, String subTitle, String Scr,
			int NotiTag, POI poi) {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(
				getApplicationContext(), NotiTag, intent, 0);
		notification.setLatestEventInfo(getApplicationContext(), subTitle, Scr,
				contentIntent);
		notificationManager.notify(NotiTag, notification);
	}

	private void tNotification(String Title, String subTitle, String Scr,
			int NotiTag) {
		NotificationManager notificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
		Notification notification = new Notification(R.drawable.ic_launcher,
				Title, System.currentTimeMillis());
	...
	private Runnable checker = new Runnable() {
		public void run() {
			...
			/*
			 * Score = priority + deadline(半小時以內10 一小時以內5 一小時以上1) +500m裡有地點 10
			 * 沒有0
			 */
			checkByTime();
			checkByPOI();
			...
			for (int i = 0; i < tmpl.size() && i < 3; i++) {
				if (tmpl.get(i).getPOI() != null) {
					...
					gNotification(tmpl.get(i).getPOI().getName(), "靠近地點", tmpl
							.get(i).getPOI().getName()
							+ "-" + tmpl.get(i).getDescription(), tmpl.get(i)
							.getNum(), tmpl.get(i).getPOI());
				} else {
					tNotification(tmpl.get(i).getDescription(), "提醒事項", tmpl
							.get(i).getDescription(), tmpl.get(i).getNum());
				}
			}
